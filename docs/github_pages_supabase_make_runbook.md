...
- `GitHub Actions` 在北京时间午盘 `11:31-13:25`、收盘 `15:02-18:20` 多批次刷新选股快照
...
- `.github/workflows/refresh_snapshot_midday.yml`
- `.github/workflows/refresh_snapshot_close.yml`

当前调度：

- 午盘主流程：北京时间 `11:31` 起，重试至 `13:25`
- 午盘备援流程：北京时间 `11:32` 起，重试至 `13:23`
- 收盘主流程：北京时间 `15:02` 起，重试至 `18:10`
- 收盘备援流程：北京时间 `15:03` 起，重试至 `18:20`
- 非交易日自动跳过
- 同一交易日的首个午盘/收盘快照生成后，后续重试只复用已有快照，避免晚到任务覆盖及时结果
