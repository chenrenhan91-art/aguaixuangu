...
# ---------------------------------------------------------------------------
# 可信新闻来源白名单
# 仅收录官方披露平台与无自营券商直接利益冲突的专业媒体。
# 东方财富旗下自有内容（东方财富Choice数据、数据宝等）不在列，
# 以防止以数据包装的营销内容混入事件驱动信号。
# ---------------------------------------------------------------------------
RELIABLE_NEWS_SOURCES: frozenset = frozenset([
    # 官方交易所 & 监管法定披露平台
    "巨潮资讯", "上海证券交易所", "深圳证券交易所", "北京证券交易所",
    # 官方证券专业媒体（四大证券报，受证监会监管）
    "证券时报", "证券时报网", "中国证券报", "中国证券报·中证网",
    "上海证券报", "上证报", "证券日报", "证券日报网",
    # 专业财经新闻机构（无自营券商，低利益冲突）
    "财联社", "第一财经", "第一财经日报",
    "21世纪经济报道", "21世纪经济",
    "每日经济新闻", "经济参考报", "经济日报", "中国经济网",
    "新华财经", "新华网", "新华社",
    "人民日报", "人民网", "人民财讯",
    "央广财经", "央视财经",
    "中新经纬", "中国新闻网", "中国基金报",
...
def _is_reliable_source(source: object) -> bool:
    """检查新闻来源是否属于可信白名单（官方平台或无直接券商利益冲突的专业媒体）。"""
    if source is None:
        return False
    s = str(source).strip()
    if not s or s == "nan":
        return False
    if s in RELIABLE_NEWS_SOURCES:
        return True
    # 子串匹配：应对"证券时报记者XX"等带后缀的来源名
    return any(r in s for r in RELIABLE_NEWS_SOURCES)
...
class AKShareMarketDataAdapter:
    ...
    def fetch_industry_rankings(self) -> pd.DataFrame:
        try:
            ...
            amount_columns = {"总成交额", "净流入"}
            for column in numeric_columns:
                if column not in enriched_df.columns:
                    continue
                if column in amount_columns:
                    enriched_df[column] = enriched_df[column].map(parse_chinese_amount)
                else:
            ...
            if not cached_df.empty:
                return cached_df
            try:
                df = run_without_proxy(ak.stock_board_industry_name_em, retries=3, retry_delay=1.0)
                df = df.copy()
                numeric_columns = [
                    "最新价",
                    "涨跌额",
                ...
                df["data_source"] = "eastmoney_industry_rankings"
                df = df.reset_index(drop=True)
                self._save_industry_rankings_cache(df, source="eastmoney_industry_rankings")
                return df
            except Exception:
                pass
            return pd.DataFrame()
    ...
    def fetch_stock_news(self, symbol: str, stock_name: str, limit: int = 8) -> pd.DataFrame:
        ...
        # 仅保留白名单可信来源，过滤掉东方财富自有平台内容（Choice数据、数据宝等）
        if "文章来源" in df.columns:
            df = df[df["文章来源"].apply(_is_reliable_source)]
        if df.empty:
            return self._empty_event_frame()
        return (
            df[
    ...
    def fetch_stock_event_feed(self, symbol: str, stock_name: str, limit: int = 12) -> pd.DataFrame:
        ...
        frames = [
            ...
            # fetch_stock_research_reports 已移除：券商报告存在系统性看多偏差，
            # 且东方财富研报与自营券商业务存在直接利益冲突，不列入事件驱动信号来源。
        ]
