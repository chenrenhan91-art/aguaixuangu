...
# A full A-share trade calendar should contain several thousand dates.
# If fewer are returned the API likely responded with empty/partial data,
# so we raise an exception to trigger the weekday-fallback branch.
_MIN_CALENDAR_DATES = 200
...
def _load_trade_dates_from_akshare() -> set[str]:
    ...
    dates = {str(value) for value in df["trade_date"].astype(str).tolist()}
    print(
        f"[check_trade_day] Sina calendar loaded: {len(dates)} dates",
        file=sys.stderr,
    )
    if len(dates) < _MIN_CALENDAR_DATES:
        raise RuntimeError(
            f"AKShare 交易日历数据量异常（仅收到 {len(dates)} 条），"
            "疑似接口返回不完整，触发工作日兜底检测。"
        )
    return dates
