...
SOURCE_BONUS = {
    # 官方交易所 & 监管法定披露平台（零利益冲突）
    "巨潮资讯": 8,
    "上海证券交易所": 7,
    "深圳证券交易所": 7,
    "北京证券交易所": 7,
    # 官方证券专业媒体（证监会监管范围，相对独立）
    "证券时报": 4,
    "证券时报网": 4,
    "中国证券报": 4,
    "中国证券报·中证网": 4,
    ...
    "界面新闻": 2,
    "澎湃新闻": 2,
    # 东方财富研报 → 已移除加分：券商报告存在系统性看多偏差，不列为可信奖励来源
}
...
class SelectionConfig:
    ...
    preliminary_candidate_count: int = 24
...
STRATEGY_MODES: tuple[StrategyModeDefinition, ...] = (
    ...
    StrategyModeDefinition(
        mode_id="ai_mainline",
        display_name="AI主线轮动",
        description="聚焦AI产业链上下游，跟踪当前热点子板块，推荐轮动受益标的。",
        holding_window="3-7D",
        score_weights={"industry": 0.35, "technical": 0.30, "event": 0.25, "sentiment": 0.10},
        preferred_features=["AI产业链", "主线轮动", "催化事件"],
    ),
    StrategyModeDefinition(
    ...
    StrategyModeDefinition(
        mode_id="low_position_defense",
        display_name="低位防守埋伏",
        description="筛选处于历史低位、下跌空间有限、无退市风险的保守标的，适合低风险建仓。",
        holding_window="10-20D",
        score_weights={"industry": 0.20, "technical": 0.38, "event": 0.32, "sentiment": 0.10},
        preferred_features=["低位蓄势", "安全边际", "基本面改善"],
    ),
    StrategyModeDefinition(
        mode_id="quality_value",
        display_name="蓝筹价值精选",
        description="优选业绩稳定、估值处于历史低位的高质量标的，兼顾大资金建仓迹象，适合中长线埋伏。",
        holding_window="15-30D",
        score_weights={"industry": 0.22, "technical": 0.30, "event": 0.38, "sentiment": 0.10},
        preferred_features=["业绩稳定", "低估值", "主力持仓"],
    ),
)
...
def build_snapshot_session(generated_at_utc: datetime) -> dict[str, str]:
    ...
    # Midday snapshot: treat any run before 14:30 Beijing as midday.
    # The 14:30 boundary gives GitHub Actions runners up to 65 minutes of
    # delay headroom beyond the last midday cron trigger (13:25 BJ) while
    # still being distinctly earlier than the close refresh window (15:05+).
    is_midday = minute_of_day < (14 * 60 + 30)
...
def should_skip_high_risk_stock(stock_name: str, row: pd.Series) -> bool:
    normalized_name = normalize_stock_name(stock_name).upper()
    if not normalized_name:
        return True
    if any(token in normalized_name for token in ("ST", "PT", "退")):
        return True

    pe_dynamic = pd.to_numeric(row.get("市盈率-动态", 0.0), errors="coerce")
    return bool(pd.notna(pe_dynamic) and float(pe_dynamic) < 0)
...
def score_industries(df: pd.DataFrame) -> pd.DataFrame:
    ...
    member_count = (scored["上涨家数"] + scored["下跌家数"]).fillna(0.0)
    total_count = member_count.replace(0, 1)
    scored["member_count"] = member_count
    scored["breadth_ratio"] = scored["上涨家数"] / total_count

    raw_amount = scored["总成交额"] if "总成交额" in scored.columns else pd.Series(dtype=float)
    amount_max = float(raw_amount.abs().max()) if not raw_amount.empty else 0.0
    if pd.isna(amount_max):
        amount_max = 0.0
    amount_proxy = raw_amount / 100_000_000 if amount_max > 10_000_000 else raw_amount

    raw_capital = scored["净流入"] if "净流入" in scored.columns else pd.Series(dtype=float)
    capital_max = float(raw_capital.abs().max()) if not raw_capital.empty else 0.0
    if pd.isna(capital_max):
        capital_max = 0.0
    capital_proxy = raw_capital / 100_000_000 if capital_max > 10_000_000 else raw_capital
    if capital_proxy.empty or float(capital_proxy.abs().sum()) == 0.0:
        capital_proxy = scored["换手率"] * 2.0

    size_factor = (scored["member_count"] / 12).clip(0.35, 1.0)
    ...
    scored["sector_score"] = (base_sector_score * (0.88 + 0.12 * size_factor) - (1 - size_factor) * 8).clip(0, 100)
...
def compute_stock_features(hist_df: pd.DataFrame) -> Optional[dict[str, float]]:
    ...
    # 涨停/跌停日振幅极小，需特殊处理避免默认 0.5 淡化信号强度
    day_range = highs[-1] - lows[-1]
    if day_range > 0.001:
        crpos = (latest_close - lows[-1]) / day_range
    elif latest_pct >= 9.0:
        crpos = 1.0   # 涨停：收盘即为日高，最优收盘质量
    elif latest_pct <= -9.0:
        crpos = 0.0   # 跌停：收盘即为日低，最差收盘质量
    else:
        crpos = 0.5
    ...
    # ── RSI 贡献（细化分区，更精准区分动量质量）──
    if 55 <= rsi14 <= 68:
        trend_score += 10     # 最佳动量区间：上涨有力未超买
    elif 45 <= rsi14 < 55:
        trend_score += 5      # 健康偏弱，仍有上行空间
    elif 68 < rsi14 <= 76:
        trend_score += 2      # 偏热区，谨慎追高
    elif 76 < rsi14 <= 83:
        trend_score -= 4      # 超买预警，回调风险上升
    elif rsi14 > 83:
        trend_score -= 8      # 严重超买，短线风险高
    elif rsi14 < 32:
        trend_score -= 4      # 超卖区，惯性下跌风险
...
def extract_event_articles(symbol: str, stock_name: str, news_df: pd.DataFrame) -> tuple[float, list[dict[str, Any]]]:
    ...
    for _, row in news_df.iterrows():
        ...
        low_signal = any(keyword in text for keyword in LOW_SIGNAL_KEYWORDS)
        announcement_noise = any(keyword in text for keyword in ANNOUNCEMENT_NOISE_KEYWORDS)
        ...
        if low_signal:
            raw_score = min(raw_score, 3.0) - 4.0

        if source_category == "官方公告":
            raw_score += 6
            if announcement_noise:
                raw_score -= 8
        elif source_category == "券商研报":
            # 券商研报存在系统性看多偏差（buy 评级约占全部评级 65%）；
            # 不予基础加分，仅在明确表达看空/下调时视为风险信号下调分数。
            if any(keyword in text for keyword in ["下调", "卖出", "减持", "中性", "谨慎"]):
                raw_score -= 5
        ...
        # 指数衰减：半衰期 28 小时，强调时效性；最低保留 15%
        # 6 小时内超新鲜信号额外加成 25%，12 小时内加成 10%
        decay = max(0.15, 0.5 ** (age_hours / 28.0))
        freshness_boost = 1.25 if age_hours <= 6 else (1.1 if age_hours <= 12 else 1.0)
        score = raw_score * decay * freshness_boost
    ...
    articles.sort(key=lambda item: abs(item["_score"]), reverse=True)
...
def build_market_regime(
    ...
    sector_weights = [max(float(sector.get("member_count", 1.0)), 1.0) for sector in top_sectors]

    def weighted_average(values: list[float]) -> float:
        weighted_pairs = [(value, weight) for value, weight in zip(values, sector_weights) if weight > 0]
    ...
    breadth_score = clip(weighted_average([sector["breadth_ratio"] * 100 for sector in top_sectors]), 0, 100)
    momentum_score = clip(weighted_average([sector["momentum_score"] for sector in top_sectors]), 0, 100)
    capital_score = clip(weighted_average([sector.get("capital_consensus_score", 50.0) for sector in top_sectors]), 0, 100)
    news_bias = 0.0
...
def build_mode_score(mode_id: str, record: dict[str, Any]) -> float:
    ...
    if mode_id == "balanced" and min(components.values()) >= 55:
        score += 5
    elif mode_id == "ai_mainline":
        ai_sectors = {"半导体", "计算机", "通信", "消费电子", "电子", "软件开发", "互联网", "元件", "军工电子"}
        if record["industry"] in ai_sectors:
            score += 6
        if record["industry_rank"] <= 3:
            score += 5
        if record["event_score"] >= 60:
            score += 5
        top_event_title = record["events"][0]["title"] if record["events"] else ""
        ai_keywords = ["AI", "算力", "芯片", "服务器", "液冷", "大模型", "智算", "云计算", "人工智能"]
        if any(kw in top_event_title for kw in ai_keywords):
            score += 7
        ...
        if feature_map.get("macd_cross_up", False):
            score += 7
    elif mode_id == "low_position_defense":
        rsi14 = feature_map.get("rsi14", 50.0)
        ret20 = feature_map.get("ret20", 0.0)
        if 25 <= rsi14 <= 45:
            score += 8
        if feature_map["latest_close"] <= feature_map["ma60"] * 1.05:
            score += 6
        if record["event_score"] >= 58:
            score += 6
        if ret20 < -0.15:
            score -= 10
        if record["sentiment_score"] >= 80:
            score -= 8
    elif mode_id == "quality_value":
        rsi14 = feature_map.get("rsi14", 50.0)
        if record["event_score"] >= 65:
            score += 8
        top_event_title = record["events"][0]["title"] if record["events"] else ""
        value_keywords = ["业绩", "净利润", "营收", "预增", "增长", "分红", "回购"]
        if any(kw in top_event_title for kw in value_keywords):
            score += 6
        if feature_map["latest_close"] <= feature_map["ma60"] * 1.03:
            score += 5
        if rsi14 <= 50:
            score += 4
        if record["sentiment_score"] >= 85:
            score -= 8
        if any(event["sentiment"] == "negative" for event in record["events"]):
            score -= 6
...
def build_adaptive_mode_thresholds(records: list[dict[str, Any]]) -> dict[str, float]:
    if not records:
        return {
            ...
            "ai_mainline_min_industry": 52.0,
            "low_position_defense_max_rsi": 55.0,
        }
    ...
    return {
        ...
        "ai_mainline_min_industry": clip(52.0 + max(avg_volatility - 4.5, 0.0) * 0.5, 52.0, 58.0),
        "low_position_defense_max_rsi": clip(55.0 - max(avg_volatility - 4.5, 0.0) * 0.5, 48.0, 55.0),
    }
...
def record_matches_mode(
    ...
    if mode_id == "balanced":
        return (
            record["industry_score"] >= 52
            and record["technical_score"] >= 50
            and record["sentiment_score"] >= 45
            and record["event_score"] >= 42
        )
    if mode_id == "ai_mainline":
        min_industry = float(thresholds.get("ai_mainline_min_industry", 52.0))
        ai_sectors = {"半导体", "计算机", "通信", "消费电子", "电子", "软件开发", "互联网", "元件", "军工电子"}
        return (
            record["industry"] in ai_sectors
            and record["industry_score"] >= min_industry
            and record["technical_score"] >= 45
            and record["event_score"] >= 40
            and record["sentiment_score"] >= 40
        )
    if mode_id == "event_driven":
        min_event_score = float(thresholds.get("event_driven_min_event", 53.0))
        return (
            record["event_score"] >= min_event_score
            and record["technical_score"] >= 45
            and record["sentiment_score"] >= 45
            and feature_map["ret10"] >= -0.05
        )
    if mode_id == "trend_breakout":
        ...
        return (
            ...
            and feature_map["latest_close"] >= feature_map["ma20"]
            and feature_map["ret10"] >= -0.02
            and feature_map["amount_ratio_5"] >= 0.8
        )
    if mode_id == "low_position_defense":
        max_rsi = float(thresholds.get("low_position_defense_max_rsi", 55.0))
        return (
            feature_map.get("rsi14", 50.0) <= max_rsi
            and feature_map.get("ret20", 0.0) >= -0.20
            and record["event_score"] >= 42
            and record["technical_score"] >= 40
            and record["sentiment_score"] <= 75
            and feature_map["latest_close"] <= feature_map["ma60"] * 1.08
        )
    if mode_id == "quality_value":
        return (
            record["event_score"] >= 50
            and record["technical_score"] >= 44
            and record["industry_score"] >= 48
            and record["sentiment_score"] <= 80
            and feature_map.get("ret20", 0.0) >= -0.15
            and not any(
                event["sentiment"] == "negative" and event["impact_level"] == "high"
...
def passes_basic_quality_gate(record: dict[str, Any]) -> bool:
    feature_map = record["feature_map"]
    return (
        record["industry_score"] >= 48
        and record["technical_score"] >= 46
        and record["sentiment_score"] >= 42
        and record["event_score"] >= 40
        and feature_map["latest_close"] >= feature_map["ma20"] * 0.94
    )
...
def build_reasons_for_mode(
    ...
    if mode_id == "ai_mainline":
        return [
            {
                "label": "AI产业链",
                "detail": f"{record['industry']} 属于AI产业链重点赛道，当前轮动强度 {record['industry_score']:.1f}。",
            },
            {
                "label": "催化事件",
                "detail": top_event or "近期有AI相关产业催化，纳入主线轮动候选池。",
            },
            {
                "label": "轮动判断",
                "detail": f"行业排名第 {record['industry_rank']} 位，技术分 {record['technical_score']:.1f}，适合跟随AI主线节奏持有。",
            },
        ]
        ...
        return [
            {
                "label": "低位特征",
                "detail": (
                    f"RSI14 {rsi14:.1f}，近 20 日收益 {feature_map['ret20'] * 100:.1f}%，"
                    "价格处于历史相对低位区间，下跌空间有限。"
                ),
            },
            {
                "label": "安全边际",
                "detail": (
                    f"当前价 {feature_map['latest_close']:.2f}，MA60 {feature_map['ma60']:.2f}，"
                    "技术面显示中长期支撑区域。"
                ),
            },
            {
                "label": "催化信号",
                "detail": top_event or "事件面无重大负面，具备低风险埋伏条件。",
            },
        ]
        ...
        return [
            {
                "label": "基本面质量",
                "detail": (
                    f"事件分 {record['event_score']:.1f}，最近催化：{top_event or '无重大负面事件'}。"
                ),
            },
            {
                "label": "价值区间",
                "detail": (
                    f"当前价 {feature_map['latest_close']:.2f} 相对 MA60 {feature_map['ma60']:.2f}，"
                    f"RSI14 {rsi14:.1f}，估值处于合理偏低区间。"
                ),
            },
            {
                "label": "中长线逻辑",
                "detail": "适合低风险建仓，主要依赖基本面改善驱动，不适合短线追涨。",
            },
        ]
...
def build_risk_plan(mode_id: str, record: dict[str, Any], market_regime: str) -> dict[str, Any]:
    ...
    stop_bounds = {
        ...
        "ai_mainline": (0.042, 0.080, 1.60, 1.85, 3.0),
        "event_driven": (0.038, 0.072, 1.4, 1.7, 2.8),
        "trend_breakout": (0.045, 0.09, 1.8, 2.0, 3.3),
        "low_position_defense": (0.055, 0.10, 2.0, 2.5, 4.0),
        "quality_value": (0.060, 0.11, 2.2, 2.8, 4.5),
    }
    ...
    base_risk_budget = {
        ...
        "low_position_defense": 0.0045,
        "quality_value": 0.0050,
    }[mode_id]
...
def build_risk_controls(mode_id: str, record: dict[str, Any]) -> list[str]:
    ...
    if mode_id == "event_driven":
        ...
        controls.append("AI板块波动较大，若行业轮动转向请优先收缩仓位。")
    elif mode_id == "low_position_defense":
        controls.append("低位策略以等待为主，切勿因短期不涨而频繁换股。")
    elif mode_id == "quality_value":
        controls.append("中长线持仓需耐心等待基本面驱动，避免追涨杀跌。")
...
def build_stock_detail(
    ...
    thesis_templates = {
        ...
        "ai_mainline": (
            f"{record['name']} 属于AI产业链 {record['industry']} 赛道，"
            f"当前行业轮动分 {record['industry_score']:.1f}，适合跟随AI主线节奏持有。"
        ),
        "event_driven": (
        ...
        "low_position_defense": (
            f"{record['name']} 当前股价处于历史低位区间，"
            f"RSI14 {record['feature_map'].get('rsi14', 50.0):.1f}，具备低风险埋伏价值。"
        ),
        "quality_value": (
            f"{record['name']} 基本面质量良好，事件分 {record['event_score']:.1f}，"
            "估值处于历史低位，适合中长线布局。"
        ),
    }
    market_context = (
...
def build_mode_payload(
    ...
    if len(eligible) < config.final_candidate_count:
        seen_symbols = {record["symbol"] for record in eligible}
        top_up = [
            record
            for record in records
            if record["symbol"] not in seen_symbols and passes_basic_quality_gate(record)
        ]
        eligible = eligible + top_up
    if not eligible:
        eligible = records[: config.final_candidate_count]
    ...
    # 行业集中度限制：同一行业最多保留 2 只，避免虚假分散化
    _sector_counts: dict[str, int] = {}
    _capped: list[dict[str, Any]] = []
    for _rec in ranked_candidates:
        _sec = _rec["industry"]
        if _sector_counts.get(_sec, 0) >= 2:
            continue
        _capped.append(_rec)
        _sector_counts[_sec] = _sector_counts.get(_sec, 0) + 1
        if len(_capped) >= config.final_candidate_count:
            break
    if len(_capped) < config.final_candidate_count:
        selected_symbols = {record["symbol"] for record in _capped}
        for _rec in ranked_candidates:
            if _rec["symbol"] in selected_symbols:
                continue
            _capped.append(_rec)
            selected_symbols.add(_rec["symbol"])
            if len(_capped) >= config.final_candidate_count:
                break
    ranked = _capped
...
def _batch_fetch_histories(
    adapter: AKShareMarketDataAdapter,
    symbols: list[str],
    max_workers: int = 8,
) -> dict[str, pd.DataFrame]:
    """使用线程池并发拉取多只股票的日线历史，将串行网络 I/O 变为并行，大幅缩短扫描时间。"""
    ...
    def _fetch_one(sym: str) -> tuple[str, pd.DataFrame]:
        try:
            return sym, adapter.fetch_stock_daily_history(sym)
        except Exception:
...
def _batch_fetch_events(
    ...
    """并发拉取候选股的事件新闻并写回 record 字典，避免串行 API 等待。
    max_workers=5 避免同时向 CNINFO / EastMoney 发起过多连接触发限流。
    """
    def _fetch_one(record: dict) -> None:
        try:
            news_df = adapter.fetch_stock_event_feed(
                symbol=record["symbol"], stock_name=record["name"], limit=12
            )
            ...
            pass  # 保持默认值 event_score=50.0, events=[]

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        list(executor.map(_fetch_one, records))


def generate_daily_candidates(config: Optional[SelectionConfig] = None) -> dict[str, Any]:
    # Capture the trigger timestamp BEFORE any data fetching so that
    # build_snapshot_session reflects when the job started (= cron trigger
    # time), not when generation finished.  A run triggered at 13:25 that
    # takes 10 min would otherwise complete at 13:35 and be mis-labelled
    # "close" instead of "midday".
    ...
    _pre_candidates: list[tuple] = []  # (sector_rank, sector, sector_name, sector_code, symbol, stock_name, row)
    ...
    for sector_rank, (_, sector) in enumerate(top_industries_df.iterrows(), start=1):
        ...
        top_sectors.append(
            {
                ...
                "member_count": int(float(sector.get("member_count", 0.0)) or 0),
            }
        )
        ...
        for _, row in constituents_df.iterrows():
            ...
            if not symbol or should_skip_high_risk_stock(stock_name, row):
                continue
            _pre_candidates.append((sector_rank, sector, sector_name, sector_code, symbol, stock_name, row))

    # 并发拉取全部候选股历史，避免串行网络等待
    _unique_symbols = list({c[4] for c in _pre_candidates})
    _history_map = _batch_fetch_histories(adapter, _unique_symbols)

    for sector_rank, sector, sector_name, sector_code, symbol, stock_name, row in _pre_candidates:
        hist_df = _history_map.get(symbol, pd.DataFrame())
    ...
    # 并发拉取预选池全量事件新闻，避免串行等待造成超时。
    # (与 _batch_fetch_histories 对称，使用线程池并发 API 请求)
    _batch_fetch_events(adapter, preliminary)
